...
export default function AddJob() {
    ...
    const addJob = async (refresh) => {
        ...
        let response = await fetch("jobs/656b857734c77ea58da7b0a0", { 
            headers, 
            method: 'POST',
    ...
    return (
        <div>
            ...
            <div className='flex flex-row items-baseline'>
                <button onClick={() => addJob(router.refresh)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">Add</button>
                <div className='ml-5'>
                    { status === true && (<p className='text-green-500'>Job added successfully.</p>) }
                    { status === false && (<p className='text-red-500'>Encountered error while adding job.</p>) }
                </div>
            </div>
        </div>
    )
